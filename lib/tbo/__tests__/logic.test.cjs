...
// --- pricing: markup from PRICE_MARKUP_PERCENT env --------------------------

test("markup defaults to 10% when PRICE_MARKUP_PERCENT is unset", () => {
  delete process.env.PRICE_MARKUP_PERCENT;
  const p = pricing.priceForCustomer({ TotalFare: 100, IsRefundable: true }, "GBP");
  assert.equal(p.sellPrice, 110);
  assert.equal(p.markupPct, 10);
});

test("PRICE_MARKUP_PERCENT overrides the markup (18% -> net*1.18)", () => {
  const prev = process.env.PRICE_MARKUP_PERCENT;
  process.env.PRICE_MARKUP_PERCENT = "18";
  try {
    const p = pricing.priceForCustomer({ TotalFare: 100, IsRefundable: true }, "GBP");
    assert.equal(p.sellPrice, 118);
    assert.equal(p.markupPct, 18);
  } finally {
    if (prev === undefined) delete process.env.PRICE_MARKUP_PERCENT;
    else process.env.PRICE_MARKUP_PERCENT = prev;
  }
});

test("invalid PRICE_MARKUP_PERCENT values fall back to 10%", () => {
  const prev = process.env.PRICE_MARKUP_PERCENT;
  try {
    for (const bad of ["abc", "-5", "500"]) {
      process.env.PRICE_MARKUP_PERCENT = bad;
      assert.equal(pricing.markupFromEnv(), 10, `expected fallback for "${bad}"`);
      const p = pricing.priceForCustomer({ TotalFare: 100, IsRefundable: true }, "GBP");
      assert.equal(p.sellPrice, 110, `expected 110 for "${bad}"`);
    }
  } finally {
    if (prev === undefined) delete process.env.PRICE_MARKUP_PERCENT;
    else process.env.PRICE_MARKUP_PERCENT = prev;
...
test("RecommendedSellingRate floor still wins over the env markup", () => {
  const prev = process.env.PRICE_MARKUP_PERCENT;
  process.env.PRICE_MARKUP_PERCENT = "18";
  try {
    const room = { TotalFare: 100, IsRefundable: true, RecommendedSellingRate: "160.67" };
    const p = pricing.priceForCustomer(room, "GBP");
    assert.equal(p.sellPrice, 160.67);
    assert.equal(p.clampedToFloor, true);
  } finally {
    if (prev === undefined) delete process.env.PRICE_MARKUP_PERCENT;
